...
所要時間の目安：STEP 1〜5 で半日〜1日、STEP 6 は +1〜2時間。
...
## STEP 6. LIFF（LINEログイン）実接続 ★唯一の追加実装

現状は「氏名を選ぶだけ」の開発用モックログインです。スマホのLINEから本人認証で
入れるようにするには、以下の実装差し替えが必要です（コード箇所は明示済み）。

1. LINE Developers で **LINEログインチャネル**を作成し、**LIFFアプリ**を追加
   （エンドポイントURL = Pagesの公開URL）。LIFF ID を控える。
2. フロント（`web`）に LIFF SDK を読み込み、起動時に `liff.init()` → `liff.getIDToken()`
   を取得して API に送る（`web/src/lib/auth.tsx` / `LoginPage.tsx` を差し替え）。
3. サーバー（`server/src/routes/auth.ts` のコメント参照）で、受け取ったIDトークンを
   `https://api.line.me/oauth2/v2.1/verify` で検証し、得られた `sub`(=lineUserId) で
   スタッフを引き当ててセッション確立に変更。
4. 初回のスタッフ紐付けは、マスター管理の「LINE userId」欄、または招待リンクからの
   初回ログインで自動保存する運用にする。

作業量の目安は 1〜2時間。ご希望あればこの STEP 6 はこちらで実装します。
