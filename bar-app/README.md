...
設定すれば実 LINE push / 共有ドライブ保存に切り替わる。LIFFの実接続（開発用モック
ログインの置き換え）のみ、認証情報とLINEログインチャネルが揃い次第の対応となる。
...
- **LINEログイン（LIFF）の実接続**: 現在は開発用モックログイン。LINEログインチャネルと
  LIFF ID を用意し、`server/src/routes/auth.ts` のコメント手順で ID トークン検証に
  差し替える（フロントは LIFF SDK 読み込み）。
- 運用しながらの調整（アラートしきい値の実データチューニング、部門別の締め時刻の
  個別設定など）。
