import type { DailyReportRep, DailyReportStaff, DepartmentGoal } from '../types.js'
import { daysInMonth, formatCurrency, monthKeyOf } from './dateUtils.js'
import {
  companyProfit,
  dailyRequiredAmount,
...
import { getAllStaff, getDepartmentGoal, getRates, getRepReportsByMonth, getStaffReportsByMonth } from '../db/repository.js'

function resolveDepartmentGoal(department: DailyReportRep['department'], month: string): DepartmentGoal {
  return getDepartmentGoal(department, month) ?? { department, month, monthlySalesGoal: 0, businessDays: daysInMonth(month) }
}

/** report は事前にリポジトリへ upsert 済みであること（月間集計に反映されている必要がある） */
export function generateRepReportText(report: DailyReportRep): string {
  const month = monthKeyOf(report.date)
  const rates = getRates()
  const staff = getAllStaff()
  const repReports = getRepReportsByMonth(report.department, month)
  const staffReports = getStaffReportsByMonth(report.department, month)

  const goal = resolveDepartmentGoal(report.department, month)
  const cumulativeSales = monthlyCumulativeSales(repReports, report.department, month)
  ...
  const staffCumulative = staffSalesCumulativeByStaff(staffReports, month)
  const staffNamesById = new Map(staff.map((s) => [s.id, s.name]))
  const hqCumulative = hqSalesCumulative(repReports, report.department, month)
  const eventCumulative = eventSalesCumulative(repReports, report.department, month)
  const expenseCumulative = monthlyExpenseCumulative(repReports, report.department, month)
  const laborCumulative = laborCostCumulative(staffReports, staff, rates, report.department, month)
  const hiaceIncentiveCumulative = hiaceIncentiveCumulativeForRep(repReports, report.reporterName, report.department, month, rates)
  const hqIncentiveCumulativeAmount = hqIncentiveCumulative(repReports, report.department, month, rates)
  const profit = monthlyProfit(repReports, staffReports, staff, rates, report.department, month)
  const personalIncentive = personalIncentiveForRep(repReports, report.reporterName, report.department, month, rates)
  const companyProfitAmount = companyProfit(repReports, staffReports, staff, rates, report.department, month, report.reporterName)
  ...
  const expenseLines = report.expenses.length
    ? report.expenses.map((e, i) => `  ${i + 1}. 費目：${e.category} / 金額：${formatCurrency(e.amount)} / 内容：${e.detail || '-'}`).join('\n')
    : '  （本日の経費入力なし）'
...
1. 個人日報
  ...
  4. ハイエース売上インセン★：${formatCurrency(hiaceIncentiveForReport(report, rates))}
...
/** report は事前にリポジトリへ upsert 済みであること */
export function generateStaffReportText(report: DailyReportStaff): string {
  const month = monthKeyOf(report.date)
  const rates = getRates()
  const staff = getAllStaff().find((s) => s.id === report.staffId)
  const staffReports = getStaffReportsByMonth(report.department, month)

  const wage = staff ? hourlyWageFor(staff, rates) : 0
  const monthlySales = monthlySalesForStaff(staffReports, report.staffId, month)
  const monthlyWorkedMinutes = workedMinutesForStaff(staffReports, report.staffId, month)
  const incentive = staffIncentiveCumulative(staffReports, report.staffId, month, rates)
  const productivity = staff ? staffProductivity(staffReports, staff, rates, month) : 0
